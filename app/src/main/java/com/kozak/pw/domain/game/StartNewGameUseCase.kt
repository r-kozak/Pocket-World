...
class StartNewGameUseCase {
    ...
    private suspend fun generateStars(starSystemsIds: List<Long>) {
        ...
        for (starSystemId in starSystemsIds) {
            ...
            // define that StarSystem contains few stars of one
            val multiStarSystem = Random.nextBoolean()
            val starsCount = if (multiStarSystem) Random.nextInt(STARS_COUNT_RANGE) else 1
            repeat(starsCount) {
